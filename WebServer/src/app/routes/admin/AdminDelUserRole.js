...
// SequelizeStore AdminDelUserRole

class AdminDelUserRole {
    ...
    constructor () {
        ...
        SYS.restifyWebServer.httpsServer.post('/api/admin/user/:id/delRole/', [
        ...
        this.JoiObjectForm = Joi.object({
            role_id: Joi.number().required().positive().invalid(2).less(4).label('Role id')
        }).label('Form')
    ...
    async _post (req, res, next) {
        ...
        if (validate != null) {
            ...
            if (isExists) {
                ...
                const destroyRoleUser = await SYS.mariadb.models.get('RoleUser').destroy({
                    where: {
                ...
                if (req.session.db.user_id === req.params.id && req.session.db.role_id === req.body.fields.role_id) {
                    req.session.db.user_id = null
                    req.session.db.role_id = null
                    ...
                    const destroySession = await SYS.mariadb.models.get('Session').destroy({
                        where: {
                            user_id: req.params.id,
                            role_id: req.body.fields.role_id
                        }
...
module.exports = AdminDelUserRole
